...
#aye aye aye -- LTER data is really annoying to work with -- hypsometry for different sampling patterns 
#for sampling mendota every 5m and at 23.5
 ME_vols_5 <- data.frame(
   ...
   mutate(ID = "ME")
#for sampling mendota at 0, 8, 20
 ME_vols_0820 <- data.frame(
   depth_layer = c("0.25-6", "7-10", "11-25"),
   vols = c(sum(ME_hypso$vols[1:7]), sum(ME_hypso$vols[8:11]), sum(ME_hypso$vols[12:26])),
   depth = c(0, 8, 20)
 ) %>%
   mutate(ID = "ME")
#for sampling mendota every 4m
 ME_vols_4 <- data.frame(
   ...
   mutate(ID = "ME")
#for sampling mendota every 4m but skipping 12m depth
...
#
   ME_2000_n <- ME_Mass_2000 %>%
     dplyr::select(sampledate, depth) %>%
     count(sampledate)
#
  ME_Mass_2000 <- ME_Mass_2000 %>%
    left_join(ME_2000_n)
...
#
 ...
 ME_up20s <- ME_up20s %>%
   left_join(ME_up20s_n, by = "sampledate") %>%
...
# #all that nonsense for monona
 #Weighting hypsometry for different sampling profiles
 
 #for sampling occuring every 4m in Lake MOnona
 MO_vols <- data.frame(
 ...
 #for sampling that skips 12m depth
 ...
 Mo_vols_0820 <- data.frame(
   depth_layer = c("0.25-6", "7-10", "11-22"),
   vols = c(sum(MO_hypso$vols[1:7]), sum(MO_hypso$vols[8:11]), sum(MO_hypso$vols[12:23])),
   depth = c(0, 8, 20)
 ) %>% mutate(ID = "MO")
...
#
 MOcl__ <- MOcl__ %>%
   left_join(MO_cl_n, by ='sampledate') %>%
   mutate(chloride_mgL = ifelse(n == 2, NA, chloride_mgL)) %>%
   mutate(depth = ifelse(depth == 4 & n == 3, 8, depth)) %>%
   drop_na(chloride_mgL)
#
