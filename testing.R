...
chloride_ts_mass <- function(chloride_data, logger_data, discharge_data) {
  ...
  ts_load <- combined %>%
    ...
    mutate(cumulative_cl_g = cumsum(cl_load_g)) #grams chloride cumulative loading
...
#function to calculate daily mass load
...
chloride_daily_mass <- function(ts_data) {
  daily <- ts_data %>%
    ...
    group_by(date) %>%
    summarise(daily_mass_kg = sum(cl_load_g)/1000) #daily load of chloride [kg]
...
chloride_monthly_load <- function(ts_data) {
  monthly <- ts_data %>%
    group_by(year_mon) %>%
    summarise(monthly_mass_Mg = sum(cl_load_g)/1000000) #monthly load of chloride [Mg]
